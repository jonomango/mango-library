...
#define enc_str(str)\
(([]() {\
	constexpr mango::EncryptedString<sizeof(str)> _encrypted_str(str);\
...
namespace mango { 
	...
	public:
		...
		explicit constexpr EncryptedString(const char(&str)[Size]) : m_key(compile_time_key(Size - 1)), m_data({}) {
